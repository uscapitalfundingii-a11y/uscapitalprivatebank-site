...
use Firebase\JWT\JWT as Whatsapp_JWT;
use Firebase\JWT\Key as Whatsapp_Key;
use WpOrg\Requests\Requests as Whatsapp_Requests;
...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =Whatsapp_JWT::decode($token, new Whatsapp_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = Whatsapp_Requests::post(VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
    ...
    {
        get_instance()->load->library('Whatsapp_aeiou');
        $module = get_instance()->app_modules->get($module_name);
        if (empty($code)) {
        ...
        $envato_res = get_instance()->whatsapp_aeiou->getPurchaseData($code);
        ...
        try {
            ...
            $response = Whatsapp_Requests::post(REG_PROD_POINT, ['Accept' => 'application/json'], $data);
