<?php
/*
Module Name: AGMC CRM Chat
Description: Chat Module for AGMC CRM
Author: AGMC LTD
Author URI: https://agmcltd.com
*/
...
function pr_chat_load_js()
{
    if (!strpos($_SERVER['REQUEST_URI'], 'chat_full_view') !== false) {
        $prChatJsPath = module_dir_path('prchat', 'assets/js/pr-chat.js');
        $prChatJsVersion = file_exists($prChatJsPath) ? filemtime($prChatJsPath) : VERSIONING;
        echo '<script src="' . module_dir_url('prchat', 'assets/js/pr-chat.js' . '?v=' . $prChatJsVersion . '') . '"></script>';
    }
    /**
     * Mentions js
     */
...
function get_customer_admins()
{
    ...
    $customer_admins = $CI->db->query('' . $select . ' ' . $where . '  OR admin=1 OR staffid IN (' . $managedAgentIds . ')) AND active=1')->result_array();
    ...
    foreach ($customer_admins as &$admin) {
        ...
        $admin['chat_last_activity'] = isset($activity[$staffId]) ? $activity[$staffId]['last_activity'] : null;
        $admin['chat_status'] = prchat_get_staff_chat_status($CI, $staffId);
        $admin['uscap_managed_agent'] = isset($managedAgentOrder[$staffId]) ? 1 : 0;
        $admin['uscap_always_online'] = isset($managedAgentOrder[$staffId]) ? 1 : 0;
    }
    unset($admin);

    usort($customer_admins, function ($a, $b) use ($activity, $managedAgentOrder) {
        $aId = (int) $a['staffid'];
        $bId = (int) $b['staffid'];

        $aCount = isset($activity[$aId]) ? (int) $activity[$aId]['message_count'] : 0;
        $bCount = isset($activity[$bId]) ? (int) $activity[$bId]['message_count'] : 0;
        $aManagedBoost = isset($managedAgentOrder[$aId]) ? 2 : 0;
        $bManagedBoost = isset($managedAgentOrder[$bId]) ? 2 : 0;
        $aScore = $aCount + $aManagedBoost;
        $bScore = $bCount + $bManagedBoost;
        if ($aScore !== $bScore) {
            return $bScore - $aScore;
        }

        $aLast = isset($activity[$aId]['last_activity']) ? strtotime((string) $activity[$aId]['last_activity']) : 0;
        $bLast = isset($activity[$bId]['last_activity']) ? strtotime((string) $activity[$bId]['last_activity']) : 0;
        if ($aLast !== $bLast) {
            return $bLast - $aLast;
        }

        if (isset($managedAgentOrder[$aId], $managedAgentOrder[$bId]) && $managedAgentOrder[$aId] !== $managedAgentOrder[$bId]) {
            return $managedAgentOrder[$aId] - $managedAgentOrder[$bId];
        }
...
/**
 * Ranks staff by recent chat communication so active client relations agents
 * float higher in the customer-facing chat roster.
 *
 ...
 * @return array
 */
function prchat_get_staff_communication_activity($CI, $staffIds)
{
    ...
    $intIds = implode(',', $staffIds);
    $staffRefs = [];
    foreach ($staffIds as $staffId) {
        $staffRefs[] = $CI->db->escape('staff_' . $staffId);
    }
    $staffRefList = implode(',', $staffRefs);
    $prefix = db_prefix();

    $sql = "
        SELECT staffid, SUM(message_count) AS message_count, MAX(last_activity) AS last_activity
        FROM (
            SELECT sender_id AS staffid, COUNT(*) AS message_count, MAX(time_sent) AS last_activity
            FROM {$prefix}chatmessages
            WHERE sender_id IN ({$intIds}) AND time_sent >= DATE_SUB(NOW(), INTERVAL 30 DAY)
            GROUP BY sender_id

            UNION ALL

            SELECT reciever_id AS staffid, COUNT(*) AS message_count, MAX(time_sent) AS last_activity
            FROM {$prefix}chatmessages
            WHERE reciever_id IN ({$intIds}) AND time_sent >= DATE_SUB(NOW(), INTERVAL 30 DAY)
            GROUP BY reciever_id

            UNION ALL

            SELECT CAST(REPLACE(sender_id, 'staff_', '') AS UNSIGNED) AS staffid, COUNT(*) AS message_count, MAX(time_sent) AS last_activity
            FROM {$prefix}chatclientmessages
            WHERE sender_id IN ({$staffRefList}) AND time_sent >= DATE_SUB(NOW(), INTERVAL 30 DAY)
            GROUP BY sender_id

            UNION ALL

            SELECT CAST(REPLACE(reciever_id, 'staff_', '') AS UNSIGNED) AS staffid, COUNT(*) AS message_count, MAX(time_sent) AS last_activity
            FROM {$prefix}chatclientmessages
            WHERE reciever_id IN ({$staffRefList}) AND time_sent >= DATE_SUB(NOW(), INTERVAL 30 DAY)
            GROUP BY reciever_id
        ) AS staff_activity
        GROUP BY staffid
    ";
...
/**
 * Reads a staff member's persisted chat status. Managed USCAP agent profiles
 * use this instead of browser presence alone so they can remain customer-visible.
 *
 * @param CI_Controller $CI
 * @param int           $staffId
 *
 * @return string
 */
function prchat_get_staff_chat_status($CI, $staffId)
{
    $CI->db->select('value');
    $CI->db->where('user_id', (int) $staffId);
    $CI->db->where('name', 'chat_status');
    $status = $CI->db->get(db_prefix() . 'chatsettings')->row('value');

    return $status ? (string) $status : 'online';
}
