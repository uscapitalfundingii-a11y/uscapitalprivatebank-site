    ...
    <script>
        ...
        function pushNewMember()
        {
            $.each(customerAdmins, function (i, admin) {
                ...
                if (user !== null || admin.uscap_always_online == 1 || String(admin.chat_status) === "online") {
                    $(".staff_container#staff_" + admin.staffid).find(".staff_children_parent_child_div").addClass("onlineStaff");
                }
            });
        }
        ...
        function removeChatMember(member)
        {
            pendingRemoves[member.id] = setTimeout(function () {
                var alwaysOnline = customerAdmins.some(function (admin) {
                    return parseInt(admin.staffid, 10) === parseInt(member.id, 10) && (admin.uscap_always_online == 1 || admin.chat_status === "online");
                });
        ...
        function appendCustomerAdmins()
        {
            ...
            $.each(customerAdmins, function (i, admin) {
                ...
                adminImage += "<div data-status=\"" + String(admin.chat_status || "online") + "\" class = \"staff_children_parent_child_div " + ((admin.uscap_always_online == 1 || String(admin.chat_status) === "online") ? "onlineStaff" : "") + "\" ><img src=\"" + fetchUserAvatar(admin.staffid, admin.profile_image) + "\" data-toggle=\"tooltip\" data-placement=\"left\" data-html=\"true\" data-original-title=\"" + fullname + "  <br>" + admin.role + " <br>Online\"/></div>";
                adminImage += "</div></div>";
