...
function init_ai_features() {
    ...
    const followUpBtn = $('#btn-ai-followup');
    const getEditor = function () {
        return tinymce.get('message') || tinymce.activeEditor;
    };
    ...
    suggestBtn.on('click', function () {
        ...
        $.post(admin_url + 'ai_tickets/suggest_reply/' + ticketId)
            .done(function (response) {
                ...
                if (result.success) {
                    setEditorContent(result.message);
                    suggestBtn
                        .button('reset')
                        .button('rephrase')
                } else {
    ...
    followUpBtn.on('click', function () {
        ...
        if (threadLength > 1500) {
            if (!confirm(app.lang.warn_ticket_thread_too_long)) {
                setTimeout(() => followUpBtn.button('reset'), 10);
                return;
            }
        }

        $.post(admin_url + 'ai_tickets/suggest_followup/' + ticketId)
            .done(function (response) {
                const result = JSON.parse(response);
                if (result.success) {
                    setEditorContent(result.message);
