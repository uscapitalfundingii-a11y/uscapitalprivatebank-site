            ...
            @if($show)
                ...
                @can($setting->route_name)
                    ...
                    <div class="col-xxl-4 col-md-6 {{ $key }} searchItems">
                        <x-widget style="2" :link="$routeName" :parameters="$params" icon="{{ $setting->icon }}" heading="{{ $setting->title }}" subheading="{{ $setting->subtitle }}" cover_cursor=1 icon_style="fill" color="primary" />
                    </div>
                @endcan
            @endif
        @endforeach
...
@push('breadcrumb-plugins')
    <div class="position-relative mb-0">
        ...
        <input class="form-control searchInput bg-white text-black" type="search" placeholder="@lang('Search')...">
    </div>
@endpush
