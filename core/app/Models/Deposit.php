...
class Deposit extends Model
{
    ...
    {
        return $query->where('deposits.method_code', '>=', 1000)->where('deposits.method_code', '<', 5000)->where('deposits.status', Status::PAYMENT_SUCCESS);
    }

    public function scopeSuccessful($query)
    {
        return $query->where('deposits.status', Status::PAYMENT_SUCCESS);
    }

    public function scopeInitiated($query)
    {
        return $query->where('deposits.status', Status::PAYMENT_INITIATE);
    }
}
