...
function ensureZoomSession($zoomModel)
{
    ...
    // Fetch Zoom API settings
    $settings = $CI->zoom_model->get_api_settings();
    if (empty($settings)) {
    ...
    // If access token is missing or invalid, refresh the token
    if (empty($accessToken) || !validateAccessToken($accessToken)) {
        if (empty($refreshToken)) {
            throw new Exception('Refresh token is missing. Please reauthorize the application.');
        }

        $newTokens = refreshZoomToken($refreshToken, $clientId, $clientSecret);
        ...
        if ($newTokens && isset($newTokens['access_token'], $newTokens['refresh_token'])) {
            ...
            saveZoomTokens($zoomModel, $settings[0]['id'], $accessToken, $refreshToken);
        } else {
            throw new Exception('Failed to refresh Zoom token. Please reauthorize the application.');
        }
    }
...
function refreshZoomToken($refreshToken, $clientId, $clientSecret)
{
    ...
    log_message('debug', 'Zoom Refresh Token Response: ' . $response);
    ...
    if ($httpCode === 401) {
        log_message('error', 'Invalid refresh token. Please reauthorize the application. Response: ' . $response);
        return null;
    }

    if ($httpCode !== 200) {
        log_message('error', 'Failed to refresh token. HTTP Code: ' . $httpCode . ' Response: ' . $response);
        return null;
    }
    ...
    log_message('error', 'Invalid refresh token response: ' . $response);
    return null;
}
...
function validateAccessToken($accessToken)
{
    if (empty($accessToken)) {
        log_message('error', 'Access token is empty.');
        return false;
    }
    ...
    if ($error) {
        log_message('error', 'Access Token Validation Error: ' . $error);
        return false;
    }

    if ($httpCode !== 200) {
        log_message('error', 'Invalid Access Token. HTTP Code: ' . $httpCode . ' Response: ' . $response);
        return false;
    }
...
function saveZoomTokens($zoomModel, $id, $accessToken, $refreshToken)
{
    $CI = &get_instance();
    $CI->load->model('zoom_model'); 
    ...
    $result = $CI->zoom_model->update_meeting_settings($id, $data);

    if ($result) {
        log_message('info', 'Zoom tokens updated successfully in the database for ID: ' . $id);
    } else {
        log_message('error', 'Failed to update Zoom tokens in the database for ID: ' . $id);
    }
}
