...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = '98F07F34';
		$this->api_url = 'https://wp.toofasthost.com/';
		$this->api_key = '1F96C90BD86DEA0FD9B9';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["0db7e4cafe020ec"])){
				$_SESSION["0db7e4cafe020ec"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["0db7e4cafe020ec"])){
				$get_data = $this->call_api(
					'POST',
